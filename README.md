...
* [`src/learn_re_com/styles.cljs`](src/learn_re_com/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/learn_re_com/`](src/learn_re_com/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/learn_re_com/core.cljs): contains the SPA entry point, `init`
* [`test/learn_re_com/`](test/learn_re_com/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `learn-re-com` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/learn_re_com/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns learn-re-com.example
  (:require [learn-re-com.config :as config])
